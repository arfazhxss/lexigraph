...
char *mstrncat(char *x, char *str2)
{                                                                         // concats dynamic string x with str2, and returns new dynamic string
    char *output = malloc(sizeof(char) * (strlen(x) + strlen(str2) + 1)); // create a dynamic array of size length(x) + length(str2) + 1

    int i = 0;
    for (i = 0; i < strlen(x); i++)
    { // copy each character of x into the dynamic string
        output[i] = x[i];
    }
    for (i = strlen(x); i < strlen(x) + strlen(str2); i++)
    { // copy each character of str2 into the dynamic string
        output[i] = str2[i - strlen(x)];
    }
    output[i] = '\0'; // add null character to the end

    free(x);       // free the dynamic space of string x
    return output; // return the dynamic string
}

char *capMAIN (char *bigSTR, char *smolSTR) {
    // printf("BIG[%s]&MONTERRY[%s]\n",bigSTR,smolSTR);
    char* local = (char*)emalloc(sizeof(char));
    char* mainLINE = (char*)emalloc(strlen(bigSTR));
    mainLINE=strdup("");
    local = strtok(bigSTR, " ");
    if (!strcasecmp(local, smolSTR)) {
        strncat(mainLINE,capStr(local),strlen(local));
        strcat(mainLINE," ");
    } 
    else {strcat(mainLINE,local);strcat(mainLINE," ");}
    while (local!=NULL) {
        local = strtok(NULL, " ");
        if ((local!=NULL)&&(!strcasecmp(local, smolSTR))) {
        //     // printf("2 ");
            // local=capStr(local);
...
int main(int argc, char *argv[]) {
    ...
    while (curr) {
        ...
        fprintf(out,"%29s",rightLIST(temp));
